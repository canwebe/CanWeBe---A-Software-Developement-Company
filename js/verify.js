...
function getdata() {
	...
	(await e.json()).forEach(e => {
		e.id === verifystring && (document.querySelector(".staffcard").style.display = "flex", verifystatus.innerHTML = "Verified", verifystatus.style.color = "#0de22a", staffname.innerHTML = e.name, staffrole.innerHTML = e.role, staffbio.innerHTML = e.bio, staffimg.setAttribute("src", e.image))
